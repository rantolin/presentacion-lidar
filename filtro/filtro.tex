...
\begin{frame}[label=filter_def]
    ...
    \begin{itemize}
 	\item 0, si el punto pertenece a un \alert<2>{objeto}
 	\item<3-> 1, si el punto pertenece al \alert<3>{terreno}
    \end{itemize}}
...
\begin{frame}
   ...
   \begin{enumerate}
    \item Hay siempre puntos anómalos debido a diferentes eventos:
     ...
     \end{itemize}
    \item \only<5>{\alert<5>{\textexclamdown\textexclamdown Deben ser eliminados!!}} \only<6->{La eliminación se hace mediante una interpolacion con spline bicúbicas bastante lisa y baja resolución.}
...
\begin{beamerboxesrounded}[shadow=true]{Definición: \emph{Borde}}
     \alt<1>{\alert<1>{NO} es un señor antipático $\Rightarrow$ \alert<1>{NO} es tu jefe}{Es una fuerte variación en altura correspondiente a una pequeña variación en planimetría}
    \end{beamerboxesrounded}
\begin{enumerate}
